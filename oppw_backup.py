...
class Sim:
    ...
    def process(self,quotes, stock, start_date, end_date, LEVERAGE, TP, SL, BE, debug, plots):
        ...
        for date in self.quotes:
            ...
            if(granular == 0 and self.balance < initial):
                    self.deposited += initial-self.balance
                    self.balance += initial-self.balance
            ...
            if(is_thursday): tpp = 0.04
            if(is_friday): tpp = 0.04
            
            if(is_monday): tpp2 = 0.017
            if(is_tuesday): tpp2 = 0.02
            if(is_wednesday): tpp2 = 0.04
            if(is_thursday): tpp2 = 0.04
            if(is_friday): tpp2 = 0.033
            ...
            if(low < open_price*SL):
                ...
                trade_type = "SL"
            elif(is_thursday and open_price > 0 and opon/open_price < 0.995):
                close_date = date
                close_price = opon
                trade_type = "TSL1"
            elif(is_thursday and open_price > 0 and low/open_price < 0.995):
                close_date = date
                close_price = open_price*0.995
                trade_type = "TSL2"
            elif(is_friday and open_price > 0 and opon/open_price < 0.995):
                close_date = date
                close_price = opon
                trade_type = "TSL3"
            elif(is_friday and open_price > 0 and low/open_price < 0.995):
                close_date = date
                close_price = open_price*0.995
                trade_type = "TSL4"
...
if __name__ == "__main__":
    ...
    result = sim.process(sim_i.quotes, "QQQ", "20000103", "20100104", LEVERAGE, tpps, SL, BE, False,True)
    result = sim.process(sim_i.quotes, "QQQ", "20100104", "20200102", LEVERAGE, tpps, SL, BE, True,True)
    result = sim.process(sim_i.quotes, "QQQ", "20220103", "20260710", LEVERAGE, 0.96, SL, BE, True,True)
    #20100104