...
class Sim:
    ...
    def sell(self, open_price, close_price, open_date, close_date, trade_type, debug=False):
                ...
                #        lev = 6
                self.gained += self.open_REG_margin*change*20
                self.balance += self.open_REG_margin*change*20
                if(debug == True):
                ...
                if(change*lev < -0.1): self.lost += self.balance*change*lev/2
    ...
    def process(self,quotes, stock, start_date, end_date, LEVERAGE, TP, SL, BE, debug, plots):
        ...
        for date in self.quotes:
            ...
            day_open = self.quotes[date][stock][930][0]
            day_high = self.quotes[date][stock][930][1]
            day_low  = self.quotes[date][stock][930][2]
            day_close = self.quotes[date][stock][1320][3]
            
            OTQ = self.quotes[date]["TQQQ"][0]
            HTQ = self.quotes[date]["TQQQ"][1]
            LTQ = self.quotes[date]["TQQQ"][2]
            CTQ = self.quotes[date]["TQQQ"][3]
            ...
            for i in range(0,candle_end+1):
                if(open_price > 0):
                    DD = low/open_price
                    if(DD < 0.095):
                        print("DD", date, i, DD)
                if(open_TQQQ_price > 0):
                    DD = low/open_TQQQ_price
                    if(DD < 0.875):
                        print("FUCK", date, i, DD)
                        gain_perc = -0.5
                        gain_value = open_TQQQ_margin * gain_perc
                        open_TQQQ_margin = 0
                        open_TQQQ_price = 0
                        open_TQQQ_date = ""
                        self.gained += gain_value
                        self.balance += gain_value
            
            if(open_price > 0):
                if(open_price*SL > day_open):
                    close_price = day_open
                    trade_type = "FUCK"
                elif(self.break_even == True):
                    if(day_open > open_price*BE):
                        close_price = day_open
                        trade_type = "BO"   
                    elif(day_high > open_price*BE):
                        close_price = open_price*BE
                        trade_type = "BH"
                
            if(day_close < open_price*BE):
            ...
            if(date_diff(prev_date, date) > 1 and (is_monday is True or is_tuesday is True)):
                if(open_price > 0):
                    close_price = self.quotes[prev_date][stock][1320][3]
                    trade_type = "TO"
                    self.sell(open_price, close_price, open_date, close_date, trade_type, debug)
                    open_price = 0
                    close_price = 0
                ...
                open_TQQQ_change = 0
                if(open_TQQQ_price > 0):
                    open_TQQQ_change = day_open/open_TQQQ_price
                balance_change = (open_TQQQ_change-1)*LEVERAGE
                self.open_REG_margin = self.balance/(20/LEVERAGE) + balance_change
                open_date  = date
                self.trade_no+=1
            if(day_low < open_price*SL):
                close_date = date
                close_price = open_price*SL
                trade_type = "SL"
            elif(day_open > open_price*(1+tpp2) and open_price > 0):
                close_date = date
                close_price = day_open
                trade_type = "OH"
            elif(day_close > open_price*(1+tpp2) and open_price > 0):
                close_date = date
            ...
            if(IBS < 0.15 and open_TQQQ_margin == 0):
                open_TQQQ_date = date
                open_change = 0
                balance_change = 0
                open_margin = 0
                open_TQQQ_price = day_close
                if(open_price > 0):
                    open_change = day_close/open_price
                    balance_change = (open_change-1)*TQQQ_leverage
                    open_margin = self.balance/(20/TQQQ_leverage) + balance_change
                balance_difference = balance_change*(self.balance/2)
                open_TQQQ_margin = (self.balance-open_margin)/(20/TQQQ_leverage)
                #print(date, self.balance, open_margin, open_TQQQ_margin)
            elif(open_TQQQ_margin > 0):
                if(date_diff(open_TQQQ_date, date) > 15):
                    TQQQ_change = day_close/open_TQQQ_price
                    gain_perc = (TQQQ_change-1)*4
                    gain_value = open_TQQQ_margin * gain_perc
                    print("TO", gain_value)
                    open_TQQQ_margin = 0
                    open_TQQQ_price = 0
                    ...
                    TQQQ_change = day_close/open_TQQQ_price
                    gain_perc = (TQQQ_change-1)*TQQQ_leverage
                    gain_value = open_TQQQ_margin * gain_perc
                    print("WIN", gain_value)
                    open_TQQQ_margin = 0
                    open_TQQQ_price = 0
                    open_TQQQ_date = ""
                    self.gained += gain_value
...
if __name__ == "__main__":
    ...
    result = sim.process(sim_i.quotes, "QQQ1", "20220102", "20251215", LEVERAGE, 0.1, SL, BE, True,True)