...
and minute strategy equity points calculated from oppw24.py results.

Place this script beside oppw24.py and quotes.pkl, then run:

    py export_oppw24_mysql.py

The default range is inclusive: 2025-01-06 through 2026-07-17.
"""
...
DEFAULT_EQUITY_BATCH_SIZE = 1000
...
@dataclass(frozen=True)
class CapturedEquityPoint:
    captured_minute: datetime
    balance: float
    equity: float
    deposit: float
...
def _bar_close(bars: Any, index: int) -> float | None:
    if not isinstance(bars, (list, tuple)) or index < 0 or index >= len(bars):
        return None
    bar = bars[index]
    if not isinstance(bar, (list, tuple)) or len(bar) < 4:
        return None
...
def minute_equity_points(trades: list[CapturedTrade], quotes: dict[str, Any], source_stock: str) -> list[CapturedEquityPoint]:
    """Build a continuous minute equity timeline from first entry to last exit.

    Open-position minutes contain mark-to-market P/L, required MT5 deposit and
    the synthetic position ticket. Flat minutes carry the latest realized
    balance with zero deposit/P/L and no position ticket. The entry minute is
    a zero-P/L baseline and the exit minute is the realized flat-account row.
    """
    if not trades:
        return []

    ordered_trades = sorted(trades, key=lambda item: (item.opened_at_utc, item.ticket))
    first_minute = ordered_trades[0].opened_at_utc.replace(second=0, microsecond=0)
    last_minute = max(trade.closed_at_utc for trade in ordered_trades).replace(second=0, microsecond=0)
    trade_index = 0
    last_realized_balance = ordered_trades[0].balance_before
    points: list[CapturedEquityPoint] = []

    for date_key in sorted(quotes):
        day = quotes.get(date_key, {})
        bars = day.get(source_stock) if isinstance(day, dict) else None
        if not isinstance(bars, (list, tuple)):
            continue
        day_first = bar_datetime_utc(date_key, FIRST_MINUTE_INDEX).replace(second=0, microsecond=0)
        day_last_index = min(LAST_SESSION_INDEX, len(bars) - 1)
        if day_last_index < FIRST_MINUTE_INDEX:
            continue
        day_last = bar_datetime_utc(date_key, day_last_index).replace(second=0, microsecond=0)
        if day_last < first_minute:
            continue
        if day_first > last_minute:
            break

        for index in range(FIRST_MINUTE_INDEX, day_last_index + 1):
            captured_minute = bar_datetime_utc(date_key, index).replace(second=0, microsecond=0)
            if captured_minute < first_minute:
                continue
            if captured_minute > last_minute:
                break

            while trade_index < len(ordered_trades) and captured_minute >= ordered_trades[trade_index].closed_at_utc.replace(second=0, microsecond=0):
                last_realized_balance = ordered_trades[trade_index].balance_after
                trade_index += 1

            active_trade = ordered_trades[trade_index] if trade_index < len(ordered_trades) else None
            is_open = active_trade is not None and active_trade.opened_at_utc.replace(second=0, microsecond=0) <= captured_minute < active_trade.closed_at_utc.replace(second=0, microsecond=0)

            if not is_open:
                points.append(
                    CapturedEquityPoint(
                        captured_minute=captured_minute,
                        balance=last_realized_balance,
                        equity=last_realized_balance,
                        deposit=0.0,
                        current_profit=0.0,
            ...
            required_deposit = active_trade.volume / 0.01 * 2240.0
            current_profit = 0.0
            if captured_minute > active_trade.opened_at_utc.replace(second=0, microsecond=0):
                mark_price = _bar_close(bars, index)
                if mark_price is None:
                    continue
                current_profit = required_deposit * 20.0 * (mark_price / active_trade.open_price - 1.0)
            points.append(
                CapturedEquityPoint(
                    captured_minute=captured_minute,
                    balance=active_trade.balance_before,
                    equity=active_trade.balance_before + current_profit,
                    deposit=required_deposit,
                    current_profit=current_profit,
                    position_ticket=active_trade.ticket,
                )
            )
...
def equity_row(point: CapturedEquityPoint, account: str) -> str:
    return "(" + ", ".join(
        [
            sql_string(account),
            sql_datetime(point.captured_minute, milliseconds=False),
            sql_number(point.balance, 4),
            sql_number(point.equity, 4),
            sql_number(point.deposit, 4),
            sql_number(point.current_profit, 4),
            str(point.position_ticket) if point.position_ticket is not None else "NULL",
        ]
    ) + ")"


def equity_insert_statements(points: list[CapturedEquityPoint], account: str, batch_size: int) -> list[str]:
    if batch_size <= 0:
        raise ValueError("Equity batch size must be positive.")
    statements: list[str] = []
    for offset in range(0, len(points), batch_size):
        ...
        statements.extend(
            [
                ...
                "VALUES",
                ",\n".join(equity_row(point, account) for point in batch),
...
def build_sql(trades: list[CapturedTrade], quotes: dict[str, Any], source_stock: str, account: str, db_symbol: str, database: str, initial_balance: float, source_name: str, start: str, end_inclusive: str, equity_batch_size: int) -> tuple[str, int]:
    if not trades:
        raise RuntimeError("No closed trades were produced for the requested range.")

    first_trade = min(trades, key=lambda trade: (trade.opened_at_utc, trade.ticket))
    first_open = first_trade.opened_at_utc
    first_open_seconds = first_open.strftime("%Y-%m-%d %H:%M:%S")
    initial_reference = "manual-initial:" + account
    initial_hash = hashlib.sha256(
        "|".join((account, first_open_seconds, "INITIAL", str(initial_balance), str(initial_balance), "MANUAL_API", initial_reference)).encode("utf-8")
    ).hexdigest()
    baseline_reference = f"historical-baseline:{account}:{first_trade.ticket}"
    baseline_hash = hashlib.sha256(
        "|".join((account, first_open_seconds, "ADJUSTMENT", str(initial_balance), str(initial_balance), "MANUAL_API", baseline_reference)).encode("utf-8")
    ).hexdigest()
    lines = [
        ...
        f"-- Minute equity rows use each {source_stock} M1 close while a trade is open and carry realized balance while flat.",
        "",
        "SET NAMES utf8mb4;",
        "SET time_zone = '+00:00';",
        ...
        "START TRANSACTION;",
        "",
        "-- Match trade-admin.php initial-balance behavior without changing immutable cash flows.",
        "INSERT IGNORE INTO account_cash_flows(strategy_key, occurred_at, flow_type, amount, balance_after, source, reference_key, note, payload_hash)",
        "SELECT " + ", ".join(
            [
                sql_string(account), sql_datetime(first_open), "'ADJUSTMENT'",
                sql_number(initial_balance, 4), sql_number(initial_balance, 4), "'MANUAL_API'",
                sql_string(baseline_reference), "'Historical baseline appended; immutable INITIAL retained'",
                sql_string(baseline_hash),
            ]
        ),
        "WHERE EXISTS (",
    ...
    equity_points = minute_equity_points(trades, quotes, source_stock)
    lines.extend([f"-- {len(equity_points)} minute strategy equity points.", ""])
    lines.extend(equity_insert_statements(equity_points, account, equity_batch_size))
    lines.extend(["COMMIT;", ""])
    return "\n".join(lines), len(equity_points)
...
def main() -> int:
    ...
    if args.equity_batch_size <= 0:
        raise SystemExit("--equity-batch-size must be positive.")
    ...
    sql, equity_point_count = build_sql(
        trades=trades,
        quotes=sim.quotes,
        source_stock=args.source_stock,
        ...
        equity_batch_size=args.equity_batch_size,
    )
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(sql, encoding="utf-8", newline="\n")

    print(f"Wrote {len(trades)} closed trades to {args.output.resolve()}")
    print(f"Wrote {equity_point_count} minute strategy equity points in batches of up to {args.equity_batch_size} rows.")
