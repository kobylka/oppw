...
# The baseline 2018-04-13 through 2026-08-04 run had a maximum winning-trade
# MAE of 4.1709%. A 4.30% breach therefore sits just outside that observed
# winning range. The trade gets until the same session's close to reclaim 4%.
DEFAULT_FAILED_RECOVERY_BREACH_DISTANCE = 0.043
DEFAULT_FAILED_RECOVERY_RECLAIM_DISTANCE = 0.04


def validate_failed_recovery_distances(breach_distance, reclaim_distance):
        """Validate the paired breach/reclaim distances for the research exit."""
        if breach_distance is None and reclaim_distance is None:
                return
        if breach_distance is None or reclaim_distance is None:
                raise ValueError("breach and reclaim distances must both be set")
        if not 0 < reclaim_distance < breach_distance < 1:
                raise ValueError(
                    "reclaim distance must be positive and smaller than breach distance"
                )


def failed_recovery_breached(open_price, bar_low, breach_distance):
        """Return whether the bar crossed the adverse-excursion threshold."""
        if breach_distance is None or open_price <= 0:
                return False
        return bar_low <= open_price * (1 - breach_distance)


def failed_recovery_close_decision(
        open_price,
        close_price,
        armed,
        reclaim_distance,
):
        """Return True when an armed trade failed to reclaim by session close."""
        if not armed:
                return False
        return close_price < open_price * (1 - reclaim_distance)
...
class Sim:
    def __init__(self):
        ...
        self.wins = []
        self.loses = []
        self.failed_recovery_armed = False
        self.failed_recovery_exits = []
    ...
    def sell(self, time, open_price, close_price, open_date, close_date, trade_type, LEVERAGE, debug=False):
                ...
                if trade_type == "FAILED_RECOVERY_CLOSE":
                    self.failed_recovery_exits.append({
                        "open_date": open_date,
    ...
    def process(
        ...
        leverage,
        tpps,
        failed_recovery_breach_distance,
        failed_recovery_reclaim_distance,
        BE,
        thursday_stop,
        friday_stop,
        ...
        for date in sorted(self.quotes):
            ...
            if open_price > 0:
                if failed_recovery_breached(
                    open_price,
                    openest,
                    failed_recovery_breach_distance,
                ):
                    self.failed_recovery_armed = True
            ...
            if open_price > 0 and close_price == 0:
                ...
                elif failed_recovery_close_decision(
                    open_price,
                    close,
                    self.failed_recovery_armed,
                    failed_recovery_reclaim_distance,
                ):
                    i = 1324
                    close_date = date
                    close_price = close
                    trade_type = "FAILED_RECOVERY_CLOSE"
                ...
                elif self.failed_recovery_armed:
                    self.failed_recovery_armed = False
...
if __name__ == "__main__":
    ...
    #    sim,
    #    LEVERAGE,
    #    DEFAULT_FAILED_RECOVERY_BREACH_DISTANCE,
    #    DEFAULT_FAILED_RECOVERY_RECLAIM_DISTANCE,
    #    BE,
    #)
    ...
    print(result)
    print("failed-recovery exits", sim.failed_recovery_exits)
