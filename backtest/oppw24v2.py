...
class Sim:
    ...
    def process(
        ...
        for date in self.quotes:
            ...
            high = max(candle[1] for candle in quotes[930:1321])
            low  = min(candle[2] for candle in quotes[930:1321])
            
            highest = max(candle[1] for candle in quotes[4:1321])
            lowest  = min(candle[2] for candle in quotes[4:1321])
            ...
            #if(open_price > 0):
            #    print(open_price,date,opon,high,low,close)
            #print(open_price,date,opon,high,low,close)
            
            if(open_price > 0):
                if(open_price*SL > lowest):
...
if __name__ == "__main__":
    ...
    print(tpps)
    result = sim.process(sim_i.quotes, "QQQ","20220103", "20251219", LEVERAGE, tpps, SL, BE, 0.005,0.015, 20000, False,True,True,False)
    print(result)
