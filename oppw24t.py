...
def validate_profit_trailing_steps(steps):
        """Validate and normalize (profit trigger, trailing distance) pairs."""
        normalized = tuple((float(trigger), float(distance)) for trigger, distance in steps)

        if not normalized:
                raise ValueError("profit_trailing_steps cannot be empty")

        previous_trigger = 0.0
        previous_distance = 1.0

        for trigger, distance in normalized:
                if not 0 < trigger < 1:
                        raise ValueError("profit trailing triggers must be between 0 and 1")
                if not 0 < distance < 1:
                        raise ValueError("profit trailing distances must be between 0 and 1")
                if trigger <= previous_trigger:
                        raise ValueError("profit trailing triggers must be strictly increasing")
                if distance > previous_distance:
                        raise ValueError("profit trailing distances must not increase")
...
def active_profit_trailing_distance(running_high, open_price, steps):
        """Return the tightest trailing distance whose profit trigger was crossed."""
        if open_price <= 0:
                return None

        reached_profit = running_high / open_price - 1.0
        active_distance = None

        for trigger, distance in steps:
                if reached_profit + 1e-12 < trigger:
                        break
...
class Sim:
    ...
    def process(
        ...
        debug=False,
        plots=False,
        profit_trailing_steps=DEFAULT_PROFIT_TRAILING_STEPS,
    ):
        self.trade_returns = []
        ...
        profit_trailing_steps = validate_profit_trailing_steps(profit_trailing_steps)

        # Each threshold selects a trailing distance measured in percentage
        # points of the entry price. Once raised, the stop never moves down.
        profit_trailing_high = 0.0
        ...
        for date in sorted(self.quotes):
            ...
            if open_price > 0:
                if profit_trailing_stop > 0 and openest <= profit_trailing_stop:
                    i = 4
                    close_date = date
                    close_price = openest
                    trade_type = "PTSGAP"
                elif open_price * SL > openest:
                    i = 4
                    close_date = date
                    close_price = openest
                    trade_type = "GAP DOWN"
            ...
            if close_price == 0 and open_price > 0:
                for i in range(4, 934):
                    ...
                    if profit_trailing_stop > 0 and o <= profit_trailing_stop:
                        close_date = date
                        close_price = o
                        trade_type = "PTSPRE"
                        break

                    elif open_price * SL > o:
                        close_date = date
                        close_price = o
                        trade_type = "SLPRE"
                        break

                    profit_trailing_high = max(profit_trailing_high, h)
                    active_trailing_distance = active_profit_trailing_distance(
                        profit_trailing_high,
                        open_price,
                        profit_trailing_steps,
                    )
                    if active_trailing_distance is not None:
                        profit_trailing_stop = max(
                            profit_trailing_stop,
                            profit_trailing_high - open_price * active_trailing_distance,
                        )

                    if profit_trailing_stop > 0 and l <= profit_trailing_stop:
                        close_date = date
                        close_price = profit_trailing_stop
                        trade_type = "PTSPRE"
                        break
                    ...
                    elif (is_thursday or is_friday) and o / open_price < thursday_SL:
                        close_date = date
                        close_price = o
                        trade_type = "TSL1PRE"
                        break
            ...
            if new_week_entry and open_price == 0:
                ...
                profit_trailing_high = open_price
                profit_trailing_stop = 0.0
                self.trade_no += 1
            ...
            if open_price > 0:
                if profit_trailing_stop > 0 and opon <= profit_trailing_stop:
                    close_date = date
                    close_price = opon
                    trade_type = "PTSO"

                elif open_price * SL > opon:
                    close_date = date
                    close_price = opon
                    trade_type = "SLO"
            ...
            if (open_price > 0 and close < open_price * BE and self.break_even is False):
...
if __name__ == "__main__":
    ...
    print(tpps)
    result = sim.process(sim_i.quotes, "QQQ","20220103", "20260716", LEVERAGE, tpps, SL, BE, 0.004,0.004, 12000, False,False,True,True)
    print(result)
