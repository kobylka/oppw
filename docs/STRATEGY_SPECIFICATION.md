...
- Every completed-session close used by entry controls, leverage selection, daily-close processing, or recovery is the close of the latest valid MT5 M1 candle whose opening timestamp is strictly before the exchange-calendar session-close boundary. For a 22:00 close this normally selects the 21:59 candle and excludes any candle opening at 22:00. Successful historical closes are cached per symbol, session date, and boundary; missing history remains retryable and fail-closed. Publisher/MySQL market points are monitoring data and never replace the executor's broker-history authority.
- `OR5` is an independently revised, Mobile-controlled open-position loss rule and defaults disabled. It evaluates only newly completed account-terminal MT5 M1 candles during the regular session. A match requires: signal-bar low / actual entry − 1 ≤ −0.50%; signal close at or below the five-minute opening-range low; and the exact trailing 60-M1 window's minimum low / first open − 1 ≤ −1.50%. Close persistence is one, so one qualifying completed close is sufficient.
- On the position entry day, OR5's trailing window cannot start before the later of cash open or the position-open minute. On carried-position days it can start at the configured same-day premarket start. All five opening-range minutes and all 60 slow-window minutes must exist exactly in that account's MT5 history; MySQL market points are never substituted.
- Startup, enablement, and a newly observed position-rule revision establish a forward-only boundary: the candle already completed before that boundary is not tested retrospectively. Missing authority or exact broker bars cannot authorize a new exit. A match must first be immutably recorded under the current global EXECUTOR fencing revision; afterward OR5 remains latched through retries, failover, or a later toggle-off and submits a market SELL using a fresh bid.
...
- DEMO TMS and REAL TMS use the fixed hard-stop ratio `0.9465` (−5.35% from the actual fill before upward whole-point/tick rounding) at both L8 and L10. Bossa retains the leverage-dependent formula `1 - 50 / (100 × leverage)`, producing `0.9375` at L8 and `0.9500` at L10. The resolved ratio applies consistently to provisional BUY protection, definitive post-fill locking, recovery, and flat-account what-if output; an existing immutable lock is never repriced merely because configuration changed.
...
- The unified 0.4% TSL becomes active at the Thursday date change. If the Thursday premarket bid is already through that threshold, the globally fenced market exit is labeled `TSL1PRE`; other crossed-threshold TSL market exits retain `TSL`. Bossa submits TSL1PRE immediately. TMS records a position-scoped immutable intent at the crossing, retains the broker hard SL, and submits no earlier than Thursday 00:01 Europe/Warsaw. A broker rejection, restart, failover, or later price recovery cannot cancel that intent; the market SELL remains retryable until the position closes.
...
| Position-rule control changes | `strategy_position_rule_control_events` |
| Position-scoped OR5 and deferred TSL1PRE exit authorization | `strategy_position_rule_trigger_events` |

`strategy_entry_rule_controls`, `strategy_entry_rule_week_state`, and `strategy_position_rule_controls` are mutable current projections backed by their immutable event tables. `strategy_trades` is a mutable projection for mobile analytics and supplies closed pre-leverage outcomes to entry control. `strategy_events` is a diagnostic stream and legacy compatibility source; neither replaces the immutable audit records.
