...
The asynchronous backend-publishing worker remains resident but fail-closed during a transient global-role lease lapse. It retains bounded queued snapshots and events, sends nothing without a valid fencing identity, and resumes after the owning runtime reacquires or renews its lease. Publisher startup also replaces an unexpectedly dead worker so an otherwise healthy Python process cannot silently stop backend delivery.
...
The same immutable position-trigger authority also retains the system-owned `TSL1PRE` intent for TMS accounts; it is not a Mobile-controlled rule and uses control revision zero. When the Thursday premarket bid crosses the newly active 0.4% TSL, the active EXECUTOR records the exact position, bid, threshold, trigger time, and not-before time before latching the exit. Bossa keeps the canonical zero delay and immediate market SELL. TMS private configurations set a 60-second delay from the Thursday Europe/Warsaw date change because their broker begins accepting orders at 00:01. Until then the existing immutable hard SL stays installed. At or after the boundary, and after any market-closed rejection, the executor retries the fresh-bid globally fenced market SELL instead of converting the intent into an exit bracket. The immutable trigger restores the same pending exit after Master-to-Backup failover even if price later recovers above the threshold.
...
Authentication endpoints live under `Mobile/backend/auth/`; push endpoints live under `Mobile/backend/push/`; administrative endpoints are not mobile read APIs. A pairing owns one stable random refresh credential stored only as an HMAC hash by the backend and in Android-Keystore-encrypted application data on the phone. Successful refresh extends the default 90-day inactivity expiry and issues a new 15-minute access token without rotating the refresh credential or revoking other unexpired access tokens. Lost responses and concurrent refreshes are therefore retryable. Explicit unpairing or administrator revocation disables the device and invalidates every refresh and access credential. Strategy decision and specification history both use the paired-device session boundary and enforce the selected account grant before reading immutable authority.
...
| Position-scoped OR5 and deferred TSL1PRE exit authorization | immutable `strategy_position_rule_trigger_events` |
