...
Filtered-performance return metrics compound every closed trade in the effective window as `product(1 + trade return) - 1`, independently for pre-leverage price returns and leveraged account returns. Their weekly geometric equivalents use `pow(compounded growth factor, 1 / effectiveRollingWeeks) - 1`; the denominator is the full effective window, including weeks without a closed trade. The API and Android model expose these values as percentage points.
