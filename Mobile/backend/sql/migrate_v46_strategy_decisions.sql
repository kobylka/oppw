-- OPPW v46: durable strategy-decision history.
-- Only strategy decisions are separated from strategy_events in this migration.
...
-- Backfill distinct decisions already embedded in historical snapshots. The
-- unique key collapses repeated snapshots carrying the same decision ID.
INSERT INTO strategy_decisions(
    ...
    symbol, side, proposed_price, proposed_volume, required_deposit, effective_leverage,
...
ON DUPLICATE KEY UPDATE
    ...
    effective_leverage = VALUES(effective_leverage), position_notional = VALUES(position_notional), sizing_units = VALUES(sizing_units),
...
DROP PROCEDURE IF EXISTS oppw_v46_add_trade_column;
DELIMITER $$
CREATE PROCEDURE oppw_v46_add_trade_column(IN column_name_value VARCHAR(64), IN definition_value TEXT)
BEGIN
    IF NOT EXISTS (
...
CALL oppw_v46_add_trade_column('decision_id', 'CHAR(32) NULL AFTER position_ticket');
CALL oppw_v46_add_trade_column('strategy_build', 'VARCHAR(160) NOT NULL DEFAULT '''' AFTER decision_id');
CALL oppw_v46_add_trade_column('parameter_hash', 'CHAR(64) NOT NULL DEFAULT '''' AFTER strategy_build');
CALL oppw_v46_add_trade_column('entry_leverage', 'DECIMAL(12,6) NULL AFTER parameter_hash');
DROP PROCEDURE oppw_v46_add_trade_column;
...
SET @decision_index_sql = IF(@decision_index_exists = 0,
    'ALTER TABLE strategy_trades ADD INDEX idx_trade_strategy_decision (strategy_key, decision_id)', 'SELECT 1');
