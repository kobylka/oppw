...
$stmt = $db->query(
    "SELECT d.device_id, d.device_name, d.enabled, d.created_at, d.last_seen_at, d.refresh_expires_at,
            ...
            MAX(COALESCE(da.can_control_service, FALSE)) AS can_control_service
...
printf("%-32s  %-24s  %-8s  %-16s  %-12s  %-24s\n", 'DEVICE ID', 'NAME', 'STATUS', 'ACCOUNTS', 'CONTROL', 'LAST SEEN');
foreach ($stmt->fetchAll() as $row) {
    printf(
        "%-32s  %-24s  %-8s  %-16s  %-12s  %-24s\n",
        $row['device_id'], substr((string)$row['device_name'], 0, 24), (bool)$row['enabled'] ? 'ACTIVE' : 'REVOKED',
        (string)($row['accounts'] ?? ''), (bool)$row['can_control_service'] ? 'ALLOWED' : 'READ-ONLY',
