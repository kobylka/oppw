...
$message = sprintf('EVENT MOBILE_RECEIPT execution_id=%s decision_id=%s position_ticket=%d latency_ms=%s', $details['execution_id'] ?: 'none', $details['decision_id'] ?: 'none', $details['position_ticket'], $latencyMs === null ? 'none' : (string)$latencyMs);
$hash = hash('sha256',$accountKey.'|'.$session['device_id'].'|'.$executionId.'|MOBILE_RECEIPT');
$statement = $db->prepare("INSERT IGNORE INTO strategy_events(strategy_key,event_time,level,name,result,message,details,event_hash) VALUES (?,?,'INFO','MOBILE_RECEIPT',TRUE,?,?,?)");
$db->beginTransaction();
try {
    ...
    error_log('OPPW mobile receipt diagnostic write failed: '.$error->getMessage());
