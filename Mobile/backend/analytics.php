...
function iso_value(?string $value): string { if(!$value)return ''; try{return atom_datetime(new DateTimeImmutable($value,new DateTimeZone('UTC')));}catch(Throwable){return $value;} }
...
function stage_millis(string $value): ?float { if($value==='')return null; try{$dt=new DateTimeImmutable($value);return (float)$dt->format('Uv');}catch(Throwable){return null;} }
...
$optionSql="SELECT entry_leverage,exit_reason,YEAR(COALESCE(closed_at,opened_at)) AS trade_year,trade_class FROM strategy_trades WHERE strategy_key IN ($placeholders)";
...
if($year!==''&&ctype_digit($year)){ $baseSql.=' AND YEAR(COALESCE(t.closed_at,t.opened_at))=?'; $params[]=(int)$year; }
...
$hasTradeSpecificFilter=$leverage!==''||$exitReason!==''||$year!==''||in_array($class,['A','B','C','D'],true);
...
$eventSql="SELECT strategy_key,event_time,result,details FROM strategy_events WHERE strategy_key IN ($placeholders) AND name='EXECUTION_STAGE' ORDER BY event_time,id";
$eventStmt=$db->prepare($eventSql);$eventStmt->execute($accountKeys);$lifecycleGroups=[];
foreach($eventStmt->fetchAll() as $row){$details=[];try{$details=json_decode((string)$row['details'],true,512,JSON_THROW_ON_ERROR);}catch(Throwable){}if(!is_array($details))continue;$executionId=trim((string)($details['execution_id']??''));if($executionId==='')continue;$stage=strtoupper((string)($details['stage']??''));if($stage==='')continue;$eventStrategyKey=(string)$row['strategy_key'];$eventDecisionId=(string)($details['decision_id']??'');$eventTicket=(int)($details['position_ticket']??0);if($hasTradeSpecificFilter&&!isset($filteredDecisionSet[$eventStrategyKey.'|'.$eventDecisionId])&&!isset($filteredTicketSet[$eventStrategyKey.'|'.$eventTicket]))continue;$entry=['stage'=>$stage,'eventAt'=>iso_value((string)($details['event_at']??$row['event_time'])),'result'=>$row['result']===null?null:(bool)$row['result'],'retcode'=>$details['retcode']??null,'fillingMode'=>(string)($details['filling_mode']??''),'referencePrice'=>n($details['reference_price']??0),'actualPrice'=>n($details['actual_price']??0),'latencyMs'=>isset($details['latency_ms'])&&is_numeric($details['latency_ms'])?(float)$details['latency_ms']:null,'reason'=>(string)($details['reason']??'')];if(!isset($lifecycleGroups[$executionId]))$lifecycleGroups[$executionId]=['executionId'=>$executionId,'strategyKey'=>(string)$row['strategy_key'],'decisionId'=>$eventDecisionId,'positionTicket'=>$eventTicket,'stages'=>[]];if($eventDecisionId!=='')$lifecycleGroups[$executionId]['decisionId']=$eventDecisionId;if($eventTicket>0)$lifecycleGroups[$executionId]['positionTicket']=$eventTicket;$lifecycleGroups[$executionId]['stages'][]=$entry;}
...
foreach($lifecycleGroups as $lifecycle){
    $stageMap=[];foreach($lifecycle['stages'] as $stage){$stageName=$stage['stage'];if(!isset($stageMap[$stageName])||!in_array($stageName,['PUBLISHED','MOBILE_RECEIPT'],true))$stageMap[$stageName]=$stage;}
    ...
    $decisionToSend=$diff('DECISION','SENT');$ack=$diff('SENT','ACCEPTED');$fill=$diff('SENT','FILLED');$protection=$diff('FILLED','PROTECTED');$publication=$diff('CLOSED','PUBLISHED');$mobile=$diff('PUBLISHED','MOBILE_RECEIPT');
...
$cashSql="SELECT strategy_key,flow_type,amount,occurred_at FROM account_cash_flows WHERE strategy_key IN ($placeholders) ORDER BY occurred_at,id";
$cashStmt=$db->prepare($cashSql);$cashStmt->execute($accountKeys);$initialByAccount=[];$topUps=0.0;$withdrawals=0.0;$cashByDay=[];
...
$equitySql="SELECT strategy_key,captured_minute,equity FROM strategy_equity_points WHERE strategy_key IN ($placeholders) ORDER BY captured_minute,id";
$equityStmt=$db->prepare($equitySql);$equityStmt->execute($accountKeys);$equityByDay=[];
...
$filterOptions=['accounts'=>array_values(array_map(fn($row)=>['key'=>(string)$row['account_key'],'label'=>(string)$row['display_name'],'type'=>(string)$row['account_type']],$allowed)),'leverages'=>array_values(array_unique(array_filter(array_map(fn($row)=>n($row['entry_leverage']??0),$optionRows),fn($value)=>$value>0))),'exitReasons'=>array_values(array_unique(array_filter(array_map(fn($row)=>(string)($row['exit_reason']??''),$optionRows),fn($value)=>$value!==''))),'years'=>array_values(array_unique(array_filter(array_map(fn($row)=>(int)($row['trade_year']??0),$optionRows),fn($value)=>$value>0))),'classes'=>['A','B','C','D']];
sort($filterOptions['leverages']);sort($filterOptions['exitReasons']);rsort($filterOptions['years']);
json_response(['ok'=>true,'generatedAt'=>atom_datetime(new DateTimeImmutable('now',new DateTimeZone('UTC'))),'filters'=>['scope'=>$scope,'account'=>$requested,'leverage'=>$leverage,'exitReason'=>$exitReason,'year'=>$year,'tradeClass'=>$class],'filterOptions'=>$filterOptions,'summary'=>$summary,'exitReasons'=>$exitReasons,'weekly'=>$weekly,'recentTrades'=>$recent,'tradeClasses'=>$classGroups,'tradeDistribution'=>['sortOrder'=>'BEST_TO_WORST','meanReturnPercent'=>avg_values(array_column($closed,'preleverageReturnPercent')),'trades'=>$distributionPoints],'rolling20'=>$rolling,'confidenceIntervals'=>$confidence,'classProfitContribution'=>$classGroups,'classDistribution'=>$classDistribution,'drawdown'=>['maxDrawdownPercent'=>abs($closedTradeMaxDrawdownPercent),'averageMaePercent'=>avg_values(array_column($closed,'maePercent')),'series'=>$drawdownSeries,'tradeKeys'=>$allSample],'parameterComparison'=>$buildComparison,'benchmark'=>$benchmark,'executionQuality'=>$executionQuality,'metricSamples'=>$metricSamples]);
