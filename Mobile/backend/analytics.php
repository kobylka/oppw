...
function geometric_return_percent(array $returns,int $periods): float { if($periods<=0)return 0.0;$growth=compounded_growth_factor($returns);return $growth>0.0?(pow($growth,1.0/$periods)-1.0)*100.0:-100.0; }
...
$summary=[
    ...
    'weeklyGeometricPreleverageReturnPercent'=>geometric_return_percent(array_map(fn($trade)=>(float)$trade['preleverageReturnPercent']/100.0,$closed),$effectiveRollingWeeks),'weeklyGeometricLeveragedReturnPercent'=>geometric_return_percent($returns,$effectiveRollingWeeks),
