...
enforce_rate_limit('analytics-device:' . (string)$session['device_id'], 12, 60);
enforce_rate_limit('analytics-ip', 30, 60);
enforce_single_flight($db, 'analytics-device', (string)$session['device_id']);
...
function compounded_growth_factor(array $returns): float { $index=1.0; foreach($returns as $return)$index*=1.0+(float)$return; return $index; }
function compounded_return_percent(array $returns): float { return (compounded_growth_factor($returns)-1.0)*100.0; }
function geometric_return_percent(array $returns): float { $closedTradeCount=count($returns);if($closedTradeCount===0)return 0.0;$growth=compounded_growth_factor($returns);return $growth>0.0?(pow($growth,1.0/$closedTradeCount)-1.0)*100.0:-100.0; }
...
$metricSamples=[
    ...
    'captureEfficiency'=>$allSample,'edgeRatio'=>$allSample,'maxDrawdown'=>[],'recoveryFactor'=>[],'consistencyScore'=>$allSample,
    ...
    'sharpeRatio'=>$allSample,'sortinoRatio'=>$allSample,'calmarRatio'=>[],'omegaRatio'=>$allSample,'ulcerIndex'=>[],'valueAtRisk95'=>$allSample,'expectedShortfall95'=>$allSample,
    'capitalAdjustedReturn'=>$allSample,'positiveWeeks'=>$allSample,'averageWeeklyProfit'=>$allSample,
    'windowCompoundedPreleverageReturn'=>$allSample,'windowCompoundedLeveragedReturn'=>$allSample,
    'weeklyGeometricPreleverageReturn'=>$allSample,'weeklyGeometricLeveragedReturn'=>$allSample,
...
$lifecycleGroups=[];
// Correlate first, filter second. Entry stages legitimately have ticket 0
// until MT5 returns the order/deal and the position becomes visible. Filtering
// those rows independently hid SIGNAL through FILLED from a ticket-filtered
// lifecycle even though a later stage in the same execution carried the
// authoritative ticket.
$appendLifecycle=function(array $row)use(&$lifecycleGroups):void{$executionId=trim((string)($row['execution_id']??''));if($executionId==='')return;$stage=strtoupper((string)($row['stage']??''));if($stage==='')return;$eventStrategyKey=(string)$row['strategy_key'];$eventDecisionId=(string)($row['decision_id']??'');$eventTicket=(int)($row['position_ticket']??0);$entry=['stage'=>$stage,'eventAt'=>iso_value((string)$row['event_time']),'result'=>$row['result']===null?null:(bool)$row['result'],'retcode'=>$row['retcode']??null,'fillingMode'=>(string)($row['filling_mode']??''),'referencePrice'=>n($row['reference_price']??0),'actualPrice'=>n($row['actual_price']??0),'latencyMs'=>isset($row['latency_ms'])&&is_numeric($row['latency_ms'])?(float)$row['latency_ms']:null,'reason'=>(string)($row['reason']??'')];if(!isset($lifecycleGroups[$executionId]))$lifecycleGroups[$executionId]=['executionId'=>$executionId,'strategyKey'=>$eventStrategyKey,'decisionId'=>'','positionTicket'=>0,'decisionIds'=>[],'positionTickets'=>[],'stages'=>[]];if($eventDecisionId!==''){if($lifecycleGroups[$executionId]['decisionId']==='')$lifecycleGroups[$executionId]['decisionId']=$eventDecisionId;$lifecycleGroups[$executionId]['decisionIds'][$eventStrategyKey.'|'.$eventDecisionId]=true;}if($eventTicket>0){if($lifecycleGroups[$executionId]['positionTicket']<=0)$lifecycleGroups[$executionId]['positionTicket']=$eventTicket;$lifecycleGroups[$executionId]['positionTickets'][$eventStrategyKey.'|'.$eventTicket]=true;}$lifecycleGroups[$executionId]['stages'][]=$entry;};
$stageLoader=static function(DateTimeImmutable $segmentStartUtc,DateTimeImmutable $segmentEndUtc,int $limit)use($db,$placeholders,$accountKeys):iterable{
    $sql="SELECT strategy_key,execution_id,decision_id,position_ticket,stage,CASE WHEN stage IN ('EXIT_FILLED','CLOSED') THEN occurred_at ELSE COALESCE(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(payload,'$.details.event_at')),''),occurred_at) END AS event_time,result,retcode,filling_mode,reference_price,actual_price,latency_ms,reason FROM strategy_execution_stages WHERE strategy_key IN ($placeholders) AND occurred_at>=? AND occurred_at<? ORDER BY occurred_at,id LIMIT ".max(1,$limit);
...
try{$stageRows=iterator_to_array(oppw_analytics_segmented_rows($cacheConfig,$segmentContext+['dataset'=>'execution-stages-v2'],$queryWindowStartUtc,$queryWindowEndUtc,$stageLoader,$segmentStats,20000,null,20000),false);}catch(OppwAnalyticsSegmentLimitException){json_response(['ok'=>false,'error'=>'Analytics lifecycle sample exceeds the safe limit'],503);}
...
foreach($lifecycleGroups as $lifecycle){
    if($hasTradeSpecificFilter){$matchesTrade=false;foreach($lifecycle['decisionIds'] as $identity=>$_){if(isset($filteredDecisionSet[$identity])){$matchesTrade=true;break;}}if(!$matchesTrade){foreach($lifecycle['positionTickets'] as $identity=>$_){if(isset($filteredTicketSet[$identity])){$matchesTrade=true;break;}}}if(!$matchesTrade)continue;}
    unset($lifecycle['decisionIds'],$lifecycle['positionTickets']);
...
$maxDrawdownCurrency=-abs((float)$drawdown['maxDrawdownCurrency']);$dailyLowMaxDrawdownPercent=abs((float)$drawdown['maxDrawdownPercent']);$ulcerIndex=(float)$drawdown['ulcerIndexPercent'];
