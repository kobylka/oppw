...
$dailyEquitySql="SELECT strategy_key,last_captured_at AS captured_minute,close_equity AS equity FROM strategy_equity_daily WHERE strategy_key IN ($placeholders)";$dailyEquityParams=$accountKeys;if($windowStartUtc!==null&&$windowEndUtc!==null){$dailyEquitySql.=' AND last_captured_at>=? AND last_captured_at<?';$dailyEquityParams[]=mysql_datetime($windowStartUtc);$dailyEquityParams[]=mysql_datetime($windowEndUtc);}
$minuteEquitySql="SELECT p.strategy_key,p.captured_minute,p.equity FROM strategy_equity_points p WHERE p.strategy_key IN ($placeholders) AND NOT EXISTS (SELECT 1 FROM strategy_equity_daily d WHERE d.strategy_key=p.strategy_key AND d.equity_day=DATE(p.captured_minute))";$minuteEquityParams=$accountKeys;if($windowStartUtc!==null&&$windowEndUtc!==null){$minuteEquitySql.=' AND p.captured_minute>=? AND p.captured_minute<?';$minuteEquityParams[]=mysql_datetime($windowStartUtc);$minuteEquityParams[]=mysql_datetime($windowEndUtc);}$equitySql="SELECT strategy_key,captured_minute,equity FROM (($dailyEquitySql) UNION ALL ($minuteEquitySql)) equity_history ORDER BY captured_minute";$equityParams=array_merge($dailyEquityParams,$minuteEquityParams);
