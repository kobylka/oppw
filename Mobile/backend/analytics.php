...
$equityStmt=$db->prepare($equitySql,[PDO::MYSQL_ATTR_USE_BUFFERED_QUERY=>false]);$equityStmt->execute($equityParams);$portfolioEquity=oppw_portfolio_equity_rows($equityStmt);$drawdown=oppw_drawdown_analyze($portfolioEquity,$cashFlows);$equityStmt->closeCursor();$drawdown['averageMaePercent']=avg_values(array_column($closed,'maePercent'));
...
$maxDrawdownCurrency=-abs((float)$drawdown['maxDrawdownCurrency']);$minuteMaxDrawdownPercent=abs((float)$drawdown['maxDrawdownPercent']);$ulcerIndex=(float)$drawdown['ulcerIndexPercent'];
