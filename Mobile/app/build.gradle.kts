...
val mobileVersion = rootProject.file("VERSION").readText().trim()
val mobileVersionParts = mobileVersion.split('.').map { it.toInt() }
require(mobileVersionParts.size == 3) { "Mobile/VERSION must use MAJOR.MINOR.PATCH" }
require(mobileVersionParts[1] in 0..99 && mobileVersionParts[2] in 0..99) {
    "Mobile/VERSION minor and patch components must each be between 0 and 99"
}
val mobileVersionCodeEpoch = 1_000_000
val mobileVersionCode = mobileVersionCodeEpoch +
    mobileVersionParts[0] * 10_000 + mobileVersionParts[1] * 100 + mobileVersionParts[2]
