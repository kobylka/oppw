...
foreach($lifecycleGroups as $lifecycle){
    ...
    $decisionToSend=$diff('DECISION','SENT');$ack=$reportedLatency('ACCEPTED')??$diff('SENT','ACCEPTED');$fill=$reportedLatency('FILLED')??$diff('SENT','FILLED');$protection=$diff('FILLED','PROTECTED');$publication=$diff('CLOSED','PUBLISHED');$mobile=$reportedLatency('MOBILE_RECEIPT')??$diff('PUBLISHED','MOBILE_RECEIPT');
